...
var job = [ 
[[100, "Warrior"], [200, "Magician"], [300, "Bowman"], [400, "Thief"], [500, "Pirate"], [1100, "Dawn Warrior"], [1200, "Blaze Wizard"], [1300, "Wind Archer"], [1400, "Night Walker"], [1500, "Thunder Breaker"], [3200, "Battle Mage"], [3300, "Wild Hunter"], [3500, "Mechanic"], [501, "Cannoneer"], [3100, "Demon Slayer"], [430, "Dual Blade"], [2002, "Mercedes"], [508, "Jett"]],
...
function action(mode, type, selection) { 
    ...
    if (status == 0) { 
        if (cm.getPlayer().getLevel() >= 10 && (cm.getPlayer().getJob() % 1000 == 0 || cm.getPlayer().getJob() == 501 || cm.getPlayer().getJob() == 507 || cm.getPlayer().getJob() == 3001 || cm.getPlayer().getJob() >= 2001 && cm.getPlayer().getJob() <= 2003) || cm.getPlayer().getLevel() >= 30 && (cm.getPlayer().getJob() % 1000 > 0 && cm.getPlayer().getJob() % 100 == 0 || cm.getPlayer().getJob() == 508) || cm.getPlayer().getLevel() >= 70 && cm.getPlayer().getJob() % 10 == 0 && cm.getPlayer().getJob() % 100 != 0 || cm.getPlayer().getLevel() >= 120 && cm.getPlayer().getJob() % 10 == 1 || cm.getPlayer().getLevel() >= 55 && cm.getPlayer().getJob() == 430)
            cm.sendYesNo("Hey! Would you like to job advance?"); 
        else { 
            cm.sendOk("You may not advance at the current state."); 
            cm.dispose(); 
        } 
    } else if (status == 1) { 
        if (cm.getPlayer().getSubcategory() == 1 && cm.getPlayer().getJob() == 430) { //Dual Blade 
            cm.getPlayer().changeJob(431); 
            cm.dispose(); 
            return; 
        } 
        if (cm.getPlayer().getSubcategory() == 1 && cm.getPlayer().getJob() == 431) { //Dual Blade 
