        ...
        if (c.getPlayer().getGMLevel() >= PlayerGMRank.SUPERGM.getLevel()) {
            ...
            switch (splitted[0].substring(1).toLowerCase()) {
                ...
                case "fakeban":
                    ...
                    World.Broadcast.broadcastMessage(CWvsContext.serverNotice(6, "<Amethyst Ban> : " + player.getName() + " has banned " + victim.getName() + " for " + InternCommand.joinStringFrom(splitted, 2)));
                    return true;
                case "unfakeban":
