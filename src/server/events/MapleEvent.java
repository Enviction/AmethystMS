...
public abstract class MapleEvent {
    ...
    public static String scheduleEvent(final MapleEventType event, final ChannelServer cserv) {
        ...
        World.Broadcast.broadcastMessage(CWvsContext.serverNotice(0, "Hey guys! Let's play a " + StringUtil.makeEnumHumanReadable(event.name()) + " event in channel " + cserv.getChannel() + "! Change to channel " + cserv.getChannel() + " and use @event command!"));
        return "";
    }
}
